...
The MCP plugin and CLI expose the same tool surface for threads, messages,
contacts, blocks, attachments, and agent lookups. Either can drive these
operations — choose based on what fits the workflow:

- Use the CLI when the user prefers direct command-line control, client
  credentials auth, or scripted automation.
- Use MCP when the host tool manages the OAuth flow automatically and the
  user wants inline tool use inside the chat session.
